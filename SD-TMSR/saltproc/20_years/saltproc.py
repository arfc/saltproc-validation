...
steps = 2434                          # 5 fuel cycle steps by default
# Isotopes description
# ID of isotope for which we want to keep constant adens (Pa-233, 232,234,235)
pa_id = np.array([1056])
th232_id = np.array([1048])             # IDs for Th-232 (90)
u233_id = np.array([1063])             # ID for U-233 (92)
pa233_id = np.array([1056])             # ID for Pa-233 (91)
# Volatile gases, interval 20 sec
kr_id = np.arange(200, 224)           # IDs for all isotopes of Kr(36)
xe_id = np.arange(690, 716)           # IDs for all isotopes of Xe(54)
# Noble metals, interval 20 sec
se_id = np.arange(155, 179)           # IDs for Selenium (34)
nob1_id = np.arange(309, 502)           # All elements from Nb(41) to Ag (47)
nob2_id = np.arange(610, 666)           # All elements from Sb(51) to Te (52)
noble_id = np.hstack((se_id, nob1_id, nob2_id))  # Stack all Noble Metals
# Seminoble metals, interval 200 days
zr_id = np.arange(288, 309)           # IDs for Zr (40)
semin_id = np.arange(502, 610)           # IDs from Cd(48) to Sn(50)
...
i_id = np.arange(666, 690)           # IDs for I(53)
vol_fluorides = np.hstack((br_id, i_id))      # Stack volatile fluorides
# Rare earth, interval 50 days
y_id = np.arange(264, 288)           # IDs for Y(39)
rees_1_id = np.arange(765, 887)           # IDs for La(57) to Sm(62)
gd_id = np.arange(908, 926)           # IDs for Gd(64)
# Stack of all Rare earth except Eu
rees_id = np.hstack((y_id, rees_1_id, gd_id))
# Eu(63)
eu_id = np.arange(887, 908)
...
parser.add_argument(
    ...
    nargs=1,
    type=int,
    default=8)         # Number of nodes -n
parser.add_argument(
    '-steps',
    nargs=1,
    type=int,
    default=2434)     # Number of steps
...
def main():
    ...
    for i in range(lasti + 1, lasti + steps + 1):
        ...
        bu_adens_arr, tank_adens_db[i, ] = pa_remove(
            pa_id, 1, bu_adens_arr, i, 1)  # Removing interval=1step=3days
